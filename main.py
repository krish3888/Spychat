from spy_detail import spy  #importing from spy_detail.py file
...
friends = [{'name':'amit' , 'age':32 , 'rating':3.4 ,'chats':[], 'is_online':True},{'name':'rohit','age':25,'rating':2.5,'chats':[],'is_online':True}]
        ...
        if salutation == "Mr." or salutation == "Ms.":  #if,else condition
            ...
            if 65 > spy['age'] > 16:  # and operator is also a solution of this
                ...
                print "Authentication complete.Welcome %s your age " \
                          "is %d and rating is %.1f .Proud to have " \
                          "you on board."%(spy['name'] , str(spy['age']) , str(spy['rating']) ) #Using placeholders
                spy_chat(spy['name'], spy['age'] , spy['rating'] )
            else:
